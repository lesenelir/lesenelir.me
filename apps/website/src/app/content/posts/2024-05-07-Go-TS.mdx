...
引用传递：传递给函数的是实参的引用地址（但肯定不是将地址传递啦，因为函数形参的类型肯定不是指针类型，
这里说地址是为了表明都是同一个堆内存实例的引用🥹），所以在函数中进行更新的时候，会修改源数据。
...
user1.print()
user1.setUsername("lesenelir changed")
user1.print()
...
（个人觉得为了统一，值接收者的方法还是要用值类型的变量进行调用，虽然 Golang 底层我们实现了**自动解引用指针**获取指针指向的值，但个人觉得这么做更不严谨、不统一😅）。
...
章节 [14-结构体和对象](#14-struct) 已经介绍了 Golang 中结构体的概念，本质来说结构体就是一种新的自定义类型。
...
Golang 隐式实现接口的好处是：实现多接口时，相对而言就更加简单了。
...
定义的时候不指定具体的类型，而是在使用的时候再指定具体的类型。
