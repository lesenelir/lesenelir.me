...
与结构体不同的是，结构体是定义了属性，而接口更多是对方法的抽象封装。
所以结构体和接口就可以明确的来表述一个对象了。接口名一般都常用 er 结尾。
...
这和 TypeScript 不同，TypeScript 中的接口如果是对类进行抽象，不仅可以描述方法还可以描述属性（但接口一般都是用来描述**方法**而不是描述属性），
