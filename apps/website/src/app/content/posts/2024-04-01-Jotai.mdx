...
作者最初是想让 Jotai 在 core 代码外可以做一个 data fetching 的功能，但是用户们希望 Jotai 能够集成 React Query 的全部功能，所以就演变出 Jotai Query。
这其中最重要的一个功能就是为了去做数据的缓存，所以作者又实现了 Jotai Cache。这样当 `userIdAtom` 的值从 2 变为 1 后，`userCacheAtom` 的值不会重新请求，而是直接从缓存中获取。

这样就存在了两种和 Jotai 结合的数据请求方案：Jotai Query 和 Jotai Cache with Atom。
